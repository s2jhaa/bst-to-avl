...
struct avl {
   ...
   void destructCode(N * &root) { //clear tree
      root= new N();
      root->r = root;
      root->rightTh = true;
      root->l = NULL;
   ...
   bool insert(N * &root, T *t,Key *key, N *lastLeft) {
      N *p = root;
      for (;;) {
         if (p->k < key->value) {
            if (p->rightTh)
...
void displayTree(N *root) { //print the tree
   N *temp = root, *p;
   for (;;) {
      p = temp;
      temp = temp->r ;
...
void avl_tree::postorder(avl *t) {
 ...
 avl* convertTree(N *root, avl_tree avl) { //print the tree
   N *temp = root, *p;
//   avl_tree avl;
